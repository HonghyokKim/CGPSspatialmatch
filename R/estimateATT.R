...
estimateATT<-function(dataset,bexp,exp.status=1,cexp,fmethod.replace=TRUE,distbuf=0.1,exp.included=FALSE,long,lat,
  ...
  if(CGPSerror !=1) {
    ...
    correxp.org<-CGPSspatialmatch::correxp(dataset,bexp,cexp,exp.status=exp.status,varinames=varilist,method=corrmethod,aftermatch="no")
    smd.m<-lapply(findat,function(data) {CGPSspatialmatch::smd(data,bexp,exp.status=exp.status,varinames=varilist)})
    correxp.m<-lapply(findat,function(data) {CGPSspatialmatch::correxp(data,bexp,paste0(cexp,"_cf"),exp.status=exp.status,varinames=varilist,method=corrmethod,weightname="weight",aftermatch="yes")})
