...
estimateATT<-function(dataset,bexp,exp.status=1,cexp,fmethod.replace=TRUE,distbuf=0.1,exp.included=FALSE,long,lat,
                     ...
                     PS.cv.nthread=1,PS.cv.subsample=1,PS.cv.eval_metric="auc",
                     PS.cv.colsample_bytree=1,PS.cv.min_child_weight=1,PS.cv.lambda=c(0),PS.cv.lambda_bias=c(0),PS.cv.alpha=c(0),
                     PS.cv.local.N=10,
                     ...
                     CGPS.cv.colsample_bytree=1,CGPS.cv.min_child_weight=1,CGPS.cv.lambda=c(0),CGPS.cv.lambda_bias=c(0),CGPS.cv.alpha=c(0),
                     CGPS.cv.local.N=10,
  ...
  if(CGPSerror !=1) {
    ...
    correxp.m<-lapply(findat,function(data) {correxp(data,bexp,paste0(cexp,"_cf"),exp.status=exp.status,varinames=varilist,method=corrmethod,weightname="weight")})
