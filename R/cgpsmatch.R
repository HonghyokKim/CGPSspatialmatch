...
#' @param exp.status the value indicating exposed units. Defalut=1
#' @param method a character string indicating the matching method used to conduct matching. Defalut is NULL. If NULL, only generalized propensity score is computed and matching is not done. Options include "nearest" (nearest neighbor matching) and "caliper" (caliper matching)
#' @param caliper_bw a numeric vector indicating caliper bandwidth. Defalut is 0.2. If method is "nearest", this parameter is ignored.
...
cgpsmatch<-function(data,bexp,cexp,model.exponly,expstatus=1,method=NULL,caliper_bw=0.2,replace=TRUE) {
  ps<-deparse(substitute(ps))
  exponly <- data[data[,bexp]==expstatus,]
  unexponly <- data[data[,bexp] != expstatus,]
  
  GPS_exponly <- dnorm(exponly[,cexp],mean=predict(model.exponly),sd=sqrt(mean(resid(model.exponly)^2)))
  GPS_exponly_Cstab <- dnorm(exponly[,cexp],mean=mean(exponly[,cexp],na.rm=T),sd=sd(exponly[,cexp],na.rm=T))
  GPS_unexponly <- dnorm(sample(predict(model.exponly,newdata=unexponly),length(nrow(unexponly)),replace=TRUE),mean=predict(model.exponly,newdata=unexponly),sd=sqrt(mean(resid(model.exponly)^2)))
